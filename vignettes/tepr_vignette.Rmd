...
```{bash raw-files, eval = FALSE, purl=FALSE}
#!/usr/bin/sh
...
```{bash cugusi-table, eval = FALSE, purl=FALSE}
#!/usr/bin/bash
...
```{r copybg, eval = FALSE, purl=FALSE}
expprepath <-  system.file("extdata", "exptab-preprocessing.csv", package="tepr")
expdfpre <- read.csv(expprepath)
bgpathvec <- sapply(expdfpre$path, function(x) system.file("extdata", x,
    package = "tepr"))
...
```{r blacklisthighmap, eval = FALSE, purl=FALSE}
...
```{r createtablescores, eval = FALSE, purl=FALSE}
...
```{r uniselect, eval = FALSE, purl=FALSE}
...
```{r uniselectonecond, eval = FALSE, purl=FALSE}
window_size > windsizethreshold &
Count_NA < countnathreshold &
meanctrl > meanctrlthreshold &
pvaltheory > pvaltheorythres
...
```{r attselect, eval = FALSE, purl=FALSE}
Universe == TRUE &
...
```{r attselectonecond, eval = FALSE, purl=FALSE}
...
```{r outgroup, eval = FALSE, purl=FALSE}
...
```{r outgroup2, eval = FALSE, purl=FALSE}
...
```{r teprmulti, eval = FALSE, purl=FALSE}
...
```{r teprmultithree, eval = FALSE, purl=FALSE}
...
```{r plotmulti, eval = FALSE, purl=FALSE}
...
```{r kneeallconds, eval = FALSE, purl=FALSE}
...
```{r completedataset, eval = FALSE, purl=FALSE}
library(tepr)
