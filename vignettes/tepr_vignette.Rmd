...
"attenuation score" with the formula: *att <- 100 - downmean / upmean * 100*.
...
The transcript of the Universe are further divided into `Group`. A transcript
belongs to the "attenuated" group if:

1) It belongs to the Universe
2) The Area Under the Curve is high enough
3) The `-log10` of the adjust p-value of the Kolmogorov-Smirnov test is above x.
...
A transcript is considered as not attenuated ("Outgroup") if:
...
The AUC values between conditions can be compared either by highlighting the
...
Kolmogorov-Smirnov test adjusted p-values and to indicate graphically the points
corresponding to a group of genes:
...
```{r kneepercent}
plothistoknee(unigroupdf, plot = TRUE)
```
