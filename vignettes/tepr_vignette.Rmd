...
If your dataset contains only one condition, either with one or more than one replicate, it is possible to analyse the data by comparing the signal to a linear "theoretical" distribution representing a uniform signal.

```{r singlecond}
expdfonecond <- expdf[which(expdf$condition == "HS" & expdf$replicate == 1), ]
message("The experiment table is limited to one condition and one replicate:")
print(expdfonecond)
```
