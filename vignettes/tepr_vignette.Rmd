...
#########
# Pre-processing - takes ~ 38 seconds
#########
...
write.csv(expdf, file = "./exptab-preprocessing.csv", row.names = FALSE, quote = FALSE)
...
finaltab <- preprocessing(expprepath, gencodepath, windsize, maptrackpath,
    blacklistpath, genomename, finaltabpath = "./", finaltabname = "anno.tsv",
...
blacklisthighmap(maptrackpath, blacklistpath, expprepath, nbcputrans = 1, allwindowsbed, windsize, genomename = "hg38", tmpfold = "./tmp")
```

### Creating the Final Table

The `?createtablescores` function merges the files stored in the previously produced `./tmp` folder that belong to the same experiment and direction. These files are combined into a single table, with two columns per experiment: one containing the experiment name and the other containing the corresponding scores. The resulting table also includes annotations for each transcript.

```{r createtablescores, eval = FALSE}
finaltab <- createtablescores(tmpfold = "./tmp", expprepath, savefinaltable = FALSE)
```
...
Using the list of results from `resteprmulti`, all plots for all comparisons can be generated using the `plotmulti` function. This function iterates through each element of `resteprmulti` (each element representing a two-condition comparison) and calls the following functions for each:

*   `plotecdf`: Generates a figure for each gene specified in `ecdfgenevec`.
*   `plotauc`: Generates figures by group and p-value. The p-value figure is not generated if `genaucvec = NA`.
*   `plotmetagenes`: Generates figures for attenuation, outgroup, universe, and all transcripts.
*   `plothistoknee`: Generates figures for percent and kb.
...
The `kneeallconds` function allows retrieval of knee values without requiring a full `teprmulti` analysis. It calculates the knee values (`knee_AUC_` and `max_diff_Fx_`) for each condition independently.
...
If your dataset contains only one condition (with one or more replicates), you can analyze the data by comparing the observed signal to a linear "theoretical" distribution representing a uniform signal. The calculation of differences in means and AUC is skipped when running `meandifference` and `allauc`. The criteria for defining the "Universe" and "Group" columns also differ (see details in the previous section).
...
Because AUC and metagene plots require two conditions, they cannot be generated in a single-condition analysis.  However, the ECDF for a given gene can be obtained as follows:
...
The histogram of knee positions can be obtained as follows:
...
For preprocessing, we provide a Conda environment that can be activated as follows:

```{bash conda, eval = FALSE}
TO DO
```

Retrieve protein-coding and long non-coding RNA (lncRNA) annotations from the Gencode file:
...
Remove blacklisted regions and generate windows for protein-coding and lncRNA files:
...
For each bedgraph file, remove blacklisted regions and low-mappability intervals, and retrieve scores for the protein-coding annotations:
...
The package provides a `joinfiles` function to generate the final table used as input for downstream analysis.  Running this function from the data directory with default parameters will produce the desired output.
