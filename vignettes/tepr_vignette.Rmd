...
done on chr13. The post-processing is performed using two experiments and
is limited to 100 transcripts. The command `system.file` retrieves the file
paths inside the "extdata" folder of the package.
...
gencodepath <- system.files("extdata", "gencode-chr13.gtf", package = "tepr")
maptrackpath <- system.files("extdata", "k50.umap.chr13.hg38.0.8.bed",
    package = "tepr")
blacklistshpath <- system.files("extdata", "hg38-blacklist-chr13.v2.bed",
    package = "tepr")

preprocessing(exppath, gencodepath, windsize, maptrackpath,
...
To run the analysis described below, the toy-example provided in the `Quick start` section is used. For the plots, the figures obtained running the functions on the complete annotations are also provided. The complete table can be downloaded at `TO DO`. You can find a note on processing the complete dataset in section 5.12. Here we will focus on 100 transcripts. 
